...
Double_t Generator::Integrand( int fNt, TLorentzVector * pf )
{
	...
	double am23 = (pf[1] + pf[2]).M();
	double am14 = (pf[0] + pf[3]).M();
	...
	double amp2_1 = 4.0*pow(ag6,2)*pow(ag7,2)*pow(am_mu,4) \
                    + ag6*ag7*(pow(ag6,2)+pow(ag7,2))*pow(am_mu,2)*prod_p1p2 \
                    + ag6*ag7*(pow(ag6,2)+pow(ag7,2))*pow(am_mu,2)*prod_p3p4 \
                    + 2.0*pow(ag6,2)*pow(ag7,2)*prod_p1p4*prod_p2p3 \
                    + (pow(ag6,4)+pow(ag7,4))*prod_p1p3*prod_p2p4;
	 
	double amp2_2 = 4.0*pow(ag6,2)*pow(ag7,2)*pow(am_mu,4) \
                    + ag6*ag7*(pow(ag6,2)+pow(ag7,2))*pow(am_mu,2)*prod_p2p3 \
                    + ag6*ag7*(pow(ag6,2)+pow(ag7,2))*pow(am_mu,2)*prod_p1p4 \
                    + 2.0*pow(ag6,2)*pow(ag7,2)*prod_p1p2*prod_p3p4 \
                    + (pow(ag6,4)+pow(ag7,4))*prod_p1p3*prod_p2p4;
	
	double amp2_3 = ag6*ag7*pow(am_mu,2)*((pow(ag6,2)+pow(ag7,2))*prod_p1p4 \
                    + (pow(ag6,2)+pow(ag7,2))*prod_p1p2 \
                    + (pow(ag6,2)+pow(ag7,2))*prod_p2p3 \
                    + (pow(ag6,2)+pow(ag7,2))*prod_p3p4 \
                    - 2.0*ag6*ag7*prod_p2p4 \
                    - 2.0*ag6*ag7*prod_p1p3 \
                    + 4.0*ag6*ag7*pow(am_mu,2)) \
                    + 2.0*(pow(ag6,4)+pow(ag7,4))*prod_p1p3*prod_p2p4;
	...
	double prop2_23 = 1.0/(pow((pow(am23,2)-pow(am_Z,2)),2)+pow((am_Z*Gam_Z),2));
	double prop2_14 = 1.0/(pow((pow(am14,2)-pow(am_Z,2)),2)+pow((am_Z*Gam_Z),2));

	const complex<double> cunity(0.0, 1.0);

	complex<double> prop_12 = 1.0/(pow(am12,2)-pow(am_Z,2)+cunity*am_Z*Gam_Z);
	complex<double> prop_34 = 1.0/(pow(am34,2)-pow(am_Z,2)+cunity*am_Z*Gam_Z);
	complex<double> prop_23 = 1.0/(pow(am23,2)-pow(am_Z,2)+cunity*am_Z*Gam_Z);
	complex<double> prop_14 = 1.0/(pow(am14,2)-pow(am_Z,2)+cunity*am_Z*Gam_Z);
	
	double amat2 = pow((pow(gemG,3)*g_HZZ),2) \
                   * (16.0*amp2_1 * prop2_12 * prop2_34 \
                    + 16.0*amp2_2 * prop2_23 * prop2_14 \
                    +  8.0*amp2_3 * real( prop_12 * prop_34 * conj(prop_23) * conj(prop_14) \
                                        + conj(prop_12) * conj(prop_34) * prop_23 * prop_14 ) );
	
	double anorm = 1.0 / (2.0*am_H) / 4.0;
...
int main()
{
	...
	int idOut[Nop] = {13,-13, 13,-13};  //mu-, mu+, mu-, mu+
